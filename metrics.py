...
from arguments import SparseParams
import math
import numpy as np
    ...
    for scene_dir in model_paths:
        try:
            ...
            for method in os.listdir(test_dir):
                ...
                print("SSIM:{:.7f}".format(torch.tensor(ssims).mean()))
                print("PSNR:{:.7f}".format(torch.tensor(psnrs).mean()))
                print("LPIPS:{:.7f}".format(torch.tensor(lpipss).mean()))
                print("AVG:{:.7f}".format(torch.tensor(avgs).mean()))
                print("")

                full_dict[scene_dir][method].update({"SSIM": torch.tensor(ssims).mean().item(),
                                                        "PSNR": torch.tensor(psnrs).mean().item(),
                                                        "LPIPS": torch.tensor(lpipss).mean().item(),
                                                        "AVG": torch.tensor(avgs).mean().item()})
