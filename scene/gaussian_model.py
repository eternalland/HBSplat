...
class GaussianModel:
    ...
    def get_propagate_from_renderdepth2(self, cam0, depth0):
    ...
    def filter_outliers3(self, sparse_args, min_loss_state):

        common_matched_data = self.common_matched_data
        cam_dict = self.view_gs
        keys = list(cam_dict.keys())
        vmask_dict = {key: {} for key in keys}
        # filter_quantile = True
        # if 'nerf_synthetic' in sparse_args.dataset or 'blender' in sparse_args.dataset:
        ...
        for i, key0 in enumerate(keys[:-1]):
            for key1 in keys[i + 1:]:
                ...
                vmask01 = vmask10 = filter_outlier.filter_outliers_dynamic(e_ij=min_loss_state[key0][key1], e_ji=min_loss_state[key1][key0],
                ...
                self.view_gs[key0]['match_infos'][key1] = {
                ...
                self.view_gs[key1]['match_infos'][key0] = {
        ...
        for key1 in keys:
            ...
            for i, key0 in enumerate(other_keys[:-1]):
                for key2 in other_keys[i + 1:]:
                    ...
                    depth_mask02 = filter_outlier.filter_outliers_dy_depth(z_i=z_val01[true_list102], z_j=z_val21[true_list120],
                                                                               switch_dynamic_filter=sparse_args.switch_dynamic_filter,
        ...
        self.common_matched_data = common_matched_data



    def second_filter2(self):
